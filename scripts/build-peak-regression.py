"""
build-peak-regression.py
Same two-feature regression architecture as build-regression.py — pred = slope_z * z
+ slope_age * age_diff + intercept — refit against peak-tools.json (best 3-yr MLB
window) instead of mlb-tools.json (career average). Same 5 tools, same predictor
stats: testing (see project notes) showed peak needs no different feature set, just
a separate fit since the outcome is noisier than career average.
Output: data/model/peak-regression.json
"""
import json, os, glob
...
TOOLS_PATH   = os.path.join(BASE, 'model', 'peak-tools.json')
REGR_PATH    = os.path.join(BASE, 'model', 'peak-regression.json')
...
print('='*75)
print('  FITTED PEAK3 MODELS PER TOOL x LEVEL x STAT  (slope_z, slope_age, intercept)')
print('='*75)
