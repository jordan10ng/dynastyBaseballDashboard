          ...
          <div style={{maxWidth:1400,margin:'0 auto',display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
            <div>
              ...
              <div style={{display:'flex',gap:'0.5rem',alignItems:'center',marginTop:'0.25rem',flexWrap:'wrap'}}>
                ...
                {LEAGUES.map(league=>{const teamName=pOwnership[league.id];const color=teamName?(teamName===MY_TEAM?'#22c55e':'#eab308'):'#ef4444';return(<div key={league.id} style={{display:'flex',alignItems:'center',gap:'0.25rem'}}><div style={{width:'6px',height:'6px',borderRadius:'50%',background:color}}/><span style={{fontSize:'0.7rem',fontFamily:'var(--font-display)',color:'var(--muted)'}}>{league.label}{teamName?` · ${teamName}`:''}</span></div>)})}
              </div>
            </div>
