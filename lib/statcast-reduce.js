...
// League-wide pull for a single calendar day (exclusive-gt/inclusive-lt, same as Savant's
// own semantics) -- everyone who played that day, in one request. A single day never gets
// near Savant's 25,000-row CSV cap (observed peak ~9.3k on a heavy day); multi-day ranges do.
function dayUrl(dateStr) {
  const prev = new Date(dateStr + 'T00:00:00Z')
  prev.setUTCDate(prev.getUTCDate() - 1)
  const gt = prev.toISOString().slice(0, 10)
  return `https://baseballsavant.mlb.com/statcast_search/csv?all=true&type=details&game_date_gt=${gt}&game_date_lt=${dateStr}`
}
