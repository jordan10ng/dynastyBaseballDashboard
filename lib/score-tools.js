...
// blendCareer's MLB-vs-MiLB blend weight, by real MLB sample size (PA for hitters, IP/BF
// for pitchers). Same magnitude as MILB_SHRINK_K -- distinct purpose (cross-source trust,
// not shrink-to-prior), starting from the same "K sample units to be meaningfully
// confident" intuition already established for this player pool. Tune independently if
// warranted once more cases are checked.
const BLEND_TRUST_K = { hitter: 200, pitcher: 80 }
...
function scoreMilbToolRaw(buckets, toolName, opts) {
  ...
  for (const b of buckets) {
    ...
    for (const stat of statKeys) {
      ...
      // Between-bucket weight = information (sample) × recency × fit quality.
      // NOT sample/(sample+K): that's a shrink-toward-prior curve and saturates, which
      // over-weights tiny samples (a 3 IP start should not rival a 100 IP season).
      // Sample-size confidence is applied once, downstream, via shrink() on total sample.
      const w = lm.corr * sample * recency
      wSum += pred * w
      wTot += w
 ...
 * @returns {object|null}
 */
// MLB sample is the ground-truth target metric; MiLB sample is only a regression-
// projected proxy for it (translation error on top of raw performance, R²≈0.1-0.5 on the
// fitted models). Raw sample-size weighting (mlbSample/(mlbSample+milbSample)) treats
// them as equally informative per unit, which lets a large recent MiLB stint dominate
// `overall` even over a thinner-but-real MLB track record -- producing overall > peak3
// for players who bounce between AAA/MLB (peak3 is real-MLB-only, so it doesn't inherit
// the inflation). Discounts MiLB's raw weight by (1 - mlbConfidence), where mlbConfidence
// saturates toward 1 as mlbSample grows past BLEND_TRUST_K, so MiLB's influence decays as
// the real MLB read gets more trustworthy -- regardless of how large the MiLB sample is.
function blendWeights(mlbSample, milbSample, k) {
  if (mlbSample + milbSample <= 0) return { mlbW: 0, milbW: 1 }
  const mlbConfidence = mlbSample / (mlbSample + k)
  const milbEffective = milbSample * (1 - mlbConfidence)
  const total = mlbSample + milbEffective
  if (total <= 0) return { mlbW: 0, milbW: 1 }
  return { mlbW: mlbSample / total, milbW: milbEffective / total }
}
...
function blendCareer(milb, mlb, opts) {
  ...
  if (type === 'two-way') {
    ...
    const { mlbW: pWa, milbW: pWb } = blendWeights(pitchSample, milbSample, BLEND_TRUST_K.pitcher)
    const { mlbW: hWa, milbW: hWb } = blendWeights(hitSample, milbSample, BLEND_TRUST_K.hitter)
    const tools = {
  ...
  const milbSample = milb._sample ?? 0
  const { mlbW, milbW } = blendWeights(mlbSample, milbSample, type === 'pitcher' ? BLEND_TRUST_K.pitcher : BLEND_TRUST_K.hitter)
