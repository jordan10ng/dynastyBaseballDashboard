...
- **Pipeline order is mandatory:** norms → mlb-tools → regression → peak-tools → peak-regression → worthy-calibration → scores → model-rank → blend-rank → call-ups. Never run `build-scores.js` without a fresh `regression.json`. Skipping `build-callups.js` doesn't error — it just silently reverts Call-Ups to whatever stale copy was last committed locally, even though GHA regenerates it correctly every night. `peak-tools`/`peak-regression`/`worthy-calibration` feed `career_blend.peak3`/`.worthy_pct`/`.worthy_actual` only — additive display stats, never read by `dynasty_score`/model-rank/blend-rank.
