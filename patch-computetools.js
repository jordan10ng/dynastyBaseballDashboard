...
const file = path.join(os.homedir(), 'Desktop/fantasy-baseball/app/players/page.tsx');
let src = fs.readFileSync(file, 'utf8');

const NEW_BLOCK = `function computeTools(player: any, mlbToolsMap: Record<string, any>): any {
  const mlbEntry = player.mlbam_id ? mlbToolsMap[String(player.mlbam_id)] : null
...
src = src.replace(
  /function computeTools\(player: any, mlbToolsMap: Record<string, any>\): any \{[\s\S]*?\n\}/,
  NEW_BLOCK
);
...
const idx = check.indexOf('function computeTools');
