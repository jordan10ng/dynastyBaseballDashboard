...
│       ├── model/pool-stats/route.ts     # Serves pool-stats.json to UI (used by tool arc)
│       ├── hot-sheet/route.ts            # Serves hot-sheet.json to UI (shape: { season, d90, d60, d30, d15, d7, generatedAt })
...
│   ├── build-fantasy-peak.py            # Peak3 projections for actual fantasy rate stats (AVG/OBP/SLG/OPS/HR-rate/3B-rate/RBI-rate/R-rate/2B-rate, WHIP/BAA/ERA/K-BB%) → data/model/fantasy-peak-regression.json. 2B-rate kept despite weak signal (~0.13 corr) — deliberate, not an oversight.
...
│       ├── hot-sheet.json                # Generated by build-scores.js. Shape: { season, d90, d60, d30, d15, d7, generatedAt }. Each key has { bats: [...], arms: [...] } — independent top-20 lists ranked by that window's delta. Each entry has deltas: { season, d90, d60, d30, d15, d7 }.
│       ├── scores-snapshot.json          # Point-in-time snapshot of model overall + _sample per player
│       └── pool-stats.json               # Per-tool pool mean/stdev of raw regression scores — used by tool arc in drawer
...
- **Hot sheet** — mobile renders delta + stacked info (name · pos/team/level · stat line · tool line). Window toggle is a `<select>` dropdown on mobile, button row on desktop.
...
- **model/fantasy-peak-regression.json** — per-stat config for the 21 fantasy rate stats (see build-fantasy-peak.py), one of three `method`s picked per-stat by testing (not a fixed rule): `direct` (`{predictors, models}`, self-predict from MiLB rates), `tool_cal` (`{tools, coef}`, maps the pool-normed peak tool grade to raw units — used only where it beats direct on both accuracy and width; SB rate is the sole survivor after hybrid testing), `hybrid` (`{predictors, models, tools, blend_coef}`, direct prediction + tool grade as an additional term — wins for 12 of 21 stats, never hurts). `scoreFantasyStat()` in build-scores.js branches on `method`; keep it in sync with any predictor/tool changes here. Rebuild by running `build-fantasy-peak.py`.
...
- **Gamelogs** — not stored locally. Game-by-game arc fetches directly from MLB Stats API on drawer open. build-scores.js also fetches CY game logs at build time for all ~400 hot sheet pool players to compute windowed deltas. `build-gamelogs.js` and local gamelog files are retired.
...
- **Tool Arc** — SVG line chart showing cumulative model score trajectory by year. Scoring matches build-scores.js exactly: recency decay (0.75^years_ago), pool norm then shrinkage, pa-based sample. Final year values match current model tiles. Respects BAT/ARM toggle for two-way players. Hover dots show per-tool values + year/level tooltip. Dynamic Y axis (min floor=88, max ceiling=110 minimum). Only shown when player has MiLB history.
...
7. `build-fantasy-peak.py` → fantasy-peak-regression.json (AVG/OBP/SLG/OPS/HR-rate/3B-rate/RBI-rate/R-rate/2B-rate, WHIP/BAA/ERA/K-BB%)
8. `build-scores.js` → model_scores on players.json + hot-sheet.json (+ career_blend.peak3/worthy_pct/worthy_actual/fantasy_peak3)
...
- **Pool stats**: build-scores.js writes `data/model/pool-stats.json` (per-tool mean/stdev of raw scores) after computing toolVals — used by the tool arc chart to normalize historical season scores onto the same scale as current model tiles
...
**Hot sheet:** Risers = rookie-eligible players whose overall(with CY) - overall(without CY) ≥ 1. Arms filtered by IP/G >= 3.0 (total games denominator, not GS — catches openers, allows RP-tagged starters). Full riser pool (~280 bats, ~115 arms) kept; windowed deltas computed for all via CY game log fetches; 6 independent top-20 lists built (season/d90/d60/d30/d15/d7) each ranked by that window's delta.

**Windowed delta computation (build-scores.js):**
- After scoring all risers, fetches CY game logs from MLB Stats API for all ~400 pool players (same two endpoints as arc: base + leagueListId=milb_all, deduped by date+opponent)
- Pre-seeds prior-year seasonCum buckets from history files (same approach as game arc in PlayerDrawer)
- For each window (7/15/30/60/90 days back from today), filters CY games to that window, accumulates into a copy of preCum, scores using same regression/pool/shrinkage math as build-scores.js
- delta = windowed_overall − exCYOverall (overall without any CY data)
- If no CY games in window → delta = null for that window (player excluded from that window's top-20)
- `deltas` stored on each riser entry: `{ season, d90, d60, d30, d15, d7 }`
...
3. ~~**Hot sheet windowed filters**~~ — **DONE** (see Fixed below)
4. **Career arc MLB points** — add MLB season(s) as arc points for graduated players, blended using same computeTools() logic as tool tiles. Requires per-season MLB tool grades, not just career totals. TODO: reconcile scoring scale between MiLB regression model and MLB z-score approach.
5. **Washout negative examples** — add ~450 clean non-graduators as negative training rows in build-regression.py to address survivorship bias. Proxy: gap>=3yr AND age>=26 AND highest level != MLB (472 candidates identified). Assign fixed outcome floor of 40 (20-80 scale) = ~98 in raw regression space (below ~101 intercept). Two approaches to evaluate: (a) add as training rows with outcome=40-equivalent, (b) separate regularization term. Raw regression intercepts ~101-102; pool norm centers at 100 stdev 15; shrinkage prior already at 88. Need to reconcile scale before implementing.
6. **Model as consensus source** — add model scores as source type M in ranking engine
7. **Roster moves** — add/drop via Fantrax API, needs session auth
8. **Trade calculator** — connect to model scores instead of placeholder exponential decay
9. **emerging** best new raw tools with X confidence
...
- build-scores.js now fetches ~800 API calls (400 players × 2 endpoints) at the end of each run for windowed deltas — adds ~2-3 min to GHA pipeline
...
- **Hot sheet windowed filters (Apr 2026):**
  - build-scores.js now fetches CY game logs for all ~400 riser pool players after scoring
  - Windowed deltas computed for season/d90/d60/d30/d15/d7 using same cumulative arc scoring logic as PlayerDrawer game arc (preCum from prior years + window games layered on top)
  - hot-sheet.json shape changed: `{ season, d90, d60, d30, d15, d7, generatedAt }` — each key is an independent top-20 list ranked by that window's delta
  - UI: Season/L90/L60/L30/L15/L7 toggle — desktop button row, mobile `<select>` dropdown
  - Each window independently surfaces different players — L7 shows who's hot right now vs Season shows YTD risers
