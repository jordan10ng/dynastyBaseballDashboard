...
- GHA only touches data files — scripts are never modified by GHA. Safe recovery:
  ...
  ```
  Then re-run the model pipeline and push normally.
...
- **model/regression.json** — per-tool per-level two-feature model. Rebuild by running `build-regression.py`.
...
- Pool re-normalization: raw regression outputs re-centered to prospect pool mean=100, stdev=15. This is necessary — the regression output range is narrow (~93-107) due to weak MiLB→MLB correlation. Renorm creates usable separation without distorting relative rankings.
...
- `age-elasticity.json` is no longer used in the pipeline — age signal is now baked into regression.json coefficients
...
- High-recency players (>50% of career sample from current season) are systematically under-valued vs consensus — model sees small sample + shrinkage, scouts already know the player. No clean fix without scout signal.
...
- **Two-feature regression (Apr 2026):**
  ...
  - Replaces previous elasticity-based z-nudge (corr_age_residual × ageDiff × 0.2) which was too weak
  ...
  - Age vs Level correlation with rank gap dropped from +0.052 to ~0.000 — age signal now correctly neutral at aggregate level
  - Old-for-level players (Campero 28, Church 25) appropriately penalized; young-for-level (Made 18, Griffin 19) appropriately rewarded
...
- GHA conflict recovery: GHA only touches data files. Pull only those files from remote, re-run model, push:
  ```bash
  git fetch origin
  git checkout origin/main -- data/history/2026.json data/players.json data/model/norms.json data/model/mlb-tools.json data/model/regression.json data/model/hot-sheet.json data/model/scores-snapshot.json
  python3 scripts/build-regression.py && node scripts/build-scores.js
